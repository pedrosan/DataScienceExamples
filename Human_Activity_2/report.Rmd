...
```{r plots-more_1, fig.width = 8, fig.height = 8, echo = FALSE, eval = FALSE}
# NOT EVALUATED
# NOT ECHOED
df <- alt.full.select 
df$user_name <- alt.user
p <- ggplot(df, aes(pitch_arm, roll_arm)) + theme_bw() + 
        geom_point(aes(col=classe)) + 
        ggtitle("pitch_arm vs. roll_arm by user and classe")
p + facet_grid(user_name ~ classe)
```

```{r plots-more_2, fig.width = 8, fig.height = 8, echo = FALSE, eval = FALSE}
# NOT EVALUATED
# NOT ECHOED
p <- ggplot(df, aes(pitch_forearm, roll_forearm)) + theme_bw() + 
        geom_point(aes(col = classe)) + 
        ggtitle("pitch_forearm vs. roll_forearm by user and classe")
p + facet_grid(user_name ~ classe)
```

<button class="toggle_plot_code">show plot code</button>
```{r plots-more_2b, fig.width = 8, fig.height = 8, echo = TRUE}
p <- ggplot(df, aes(pitch_arm, roll_arm)) + theme_bw() + 
            theme(legend.position = "top", 
                  panel.background = element_rect(fill = "grey90")) +
            scale_color_gradient(low = "gold", high = "blue") +
            geom_point(aes(col = total_accel_arm)) + 
            ggtitle("pitch_arm vs. roll_arm by user and classe")
p + facet_grid(user_name ~ classe)
```

<button class="toggle_plot_code">show plot code</button>
```{r plots-more_1b, fig.width = 8, fig.height = 8, echo = TRUE}
p <- ggplot(df, aes(pitch_forearm, roll_forearm)) + theme_bw() + 
            theme(legend.position = "top", 
                  panel.background = element_rect(fill = "grey90")) +
            scale_color_gradient(low = "gold", high = "blue") +
            geom_point(aes(col = total_accel_forearm)) + 
            ggtitle("pitch_forearm vs. roll_forearm by user and classe")
p + facet_grid(user_name ~ classe)
```
