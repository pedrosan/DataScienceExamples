...
```{r perl_example_2, engine = "bash", eval = FALSE}
...
<button class="toggle_code">show code</button>
```{r data-basic_stats, eval = TRUE, cache = TRUE, echo = TRUE}
...
```{r data-basic_stats-print, eval = FALSE, echo = FALSE}
# NOT EVALUATED
# NOT ECHOED
stats.df
```
...
is shown in the [__Appendix__](#APPENDIX-my_functions)).
...
stats.df[5, ] <- round(stats.df[4, ]/stats.df[1, ], 3)
row.names(stats.df)[4] <- "sentences"
row.names(stats.df)[5] <- "sentences_per_line"
```
...
for(i in 1:4){ stats.df[i, ] <- sprintf("%s", stats.df[i, ]) }
kable(stats.df, align = "r", row.names = TRUE)
```
...
I have been extracting _n-grams_ for $n = 3, 4, 5$.   
...
So, I am processing data in chunks of 100,000 sentences, as _exemplified_ by this block of code
(the _n-grams_ data for the following section are loaded from saved previous analysis).


```{r ngrams-1, eval = FALSE, cache = TRUE, echo = TRUE}
# NOT EVALUATED because too computationally heavy (loading saved products)
token_delim <- " \\t\\r\\n.!?,;\"()"
nl.chunk <- 100000
...
The chunks of n-grams are then combined with a script:

```{r combine_nXgrams, eval = FALSE, cache = TRUE, echo = TRUE}
source("./scripts/combine_nXgrams_blogs.R")
```

```{r read_combine_nXgrams, eval = TRUE, echo = FALSE, cache = FALSE}
read_chunk("./scripts/combine_nXgrams_blogs.R")
```

<button class="toggle_code">show script code</button>
```{r example_of_combine_nXgrams_script, eval = FALSE, echo = TRUE, cache = FALSE}
<<combine_nXgrams_blogs>>
```
...
```{r ngrams_blogs_1, eval = TRUE, cache = TRUE, echo = TRUE}
...
```{r ngrams_news_1, eval = TRUE, cache = TRUE, echo = TRUE}
...
```{r ngrams_twitter_1, eval = TRUE, cache = TRUE, echo = TRUE}
...
tmp3.df <- cbind(head(n3g.blogs.freq, 20), head(n3g.news.freq, 20), head(n3g.twitter.freq, 20)) 
tmp4.df <- cbind(head(n4g.blogs.freq, 20), head(n4g.news.freq, 20), head(n4g.twitter.freq, 20)) 
...
```{r barplot_mix, eval = TRUE, cache = TRUE, echo = TRUE, fig.width = 10, fig.height = 7}
# ECHO FALSE
...
<button class="toggle_plot_code">show plot code</button>
```{r barplot_together, eval = TRUE, cache = TRUE, echo = TRUE, fig.width = 7, fig.height = 15}
# ECHO FALSE
...
* [scripts/combine_nXgrams_blogs.R](./scripts/combine_nXgrams_blogs.R) 
* [scripts/combine_nXgrams_news.R](./scripts/combine_nXgrams_news.R) 
* [scripts/combine_nXgrams_twitter.R](./scripts/combine_nXgrams_twitter.R) 
