...
#-----------------------------------------------------------
# Original format had ";" separated columns as:
#   freq;w1;...;wN;pred
#
# NEW format has ";" separated columns as:
#   Nsum;N;freq;w1;...;wN;pred
#-----------------------------------------------------------
open(NGRAMS3, "./N3GRAMS_2_5_with_root_stats");
...
while (<NGRAMS3>){
    ...
    chomp;
    #$full = $_;
    ($sum, $count, $freq, @words) = split(";");
    $ngram = join(";", @words);
    #$n3grams_full{"$ngram"} = $full;
...
#-----------------------------------------------------------
open(NGRAMS4, "./N4GRAMS_2_5_with_root_stats");
...
while (<NGRAMS4>){
    ...
    #$g43_freq{"$sub3"} += $freq;
    $g43_count{"$sub3"} ++ ;
    $g43_cumfreq{"$sub3"} += $freq;
}
close(NGRAMS4);
printf "# Loaded 4-grams : %d\n", $N4;

#-----------------------------------------------------------
open(NGRAMS5, "./N5GRAMS_2_5_with_root_stats");
