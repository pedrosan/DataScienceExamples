...
while (<STDIN>) {
    ...
    if( $flag_save_to_file ) {
        # It EXPORTS in a new format with log(Prob) 
        #  - only n-grams whose 'pred' is included in the reduced dictionary.
    ...
    if( $flag_save_filtered_ngrams ) {
        # It REPRINTS only n-grams whose 'pred' is included in the reduced dictionary.
        if( defined $ID{"$pred"} && $flag{"$pred"} =~ "OK" ) { 
