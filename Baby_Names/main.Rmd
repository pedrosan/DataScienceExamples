---
title: "Coding Challenge : Part 2 > Baby Names"
author: "Giovanni Fossati"
output:
  html_document:
    css: css/baby_names.css
    highlight: tango
    keep_md: yes
    mathjax: default
    self_contained: yes
...
```
```{r more_preparation, cache = TRUE, echo = FALSE}
...
```{r plot_power_law_distribution_example_x4, fig.width = 7, fig.height = 4, echo = FALSE}
...
```{r q3-most_ambiguous_names-prepare_data, echo = FALSE}
...
```{r q3-most_ambiguous_2013-method1, echo = FALSE}
...
```{r q3-most_ambiguous_2013-method2a, echo = FALSE}
...
```{r q3-most_ambiguous_2013-method3, echo = FALSE}
...
```{r q3-most_ambiguous_2013-method3-plot, echo = FALSE, fig.width = 6.0, fig.height = 6.0}
...
```{r q3-most_ambiguous_1945-method3_v2, echo = FALSE}
top1945 <- filter(c1945, diff_norm <= rn) %>% arrange(desc(N_tot)) %>% as.data.frame(.) %>% head( . , N_show)
kable(top1945, row.names = TRUE)
```

```{r q3-most_ambiguous_1945-method3-plot, echo = FALSE, fig.width = 6.0, fig.height = 6.0}
par(gr_par)
...
points(c1945$N_female[c1945$diff_norm <= abs(rn)], c1945$N_male[c1945$diff_norm <= abs(rn)], pch = 23, col = "darkgreen", bg = rgb(0, 0.9, 0, 0.3))
points(c1945$N_female[c1945$diff == 0], c1945$N_male[c1945$diff == 0], pch = 23, col = "blue2", bg = rgb(0, 0, 0.9, 0.3))
lines(x, y1, lty = 4, lwd = 1.5, col = "forestgreen")
lines(x, y2, lty = 4, lwd = 1.5, col = "forestgreen")
grid()
text(lab_x, lab_y, labels = lab_s, pos = lab_pos, cex = 0.9, adj = c(NA, 0))
...
```{r q4-largest_changes-prepare_data, echo = FALSE}
...
```{r q4-largest_change-prepare_data-exclusive_and_common_names, echo = FALSE}
...
```{r q4-largest_increase_table, echo = FALSE}
...
```{r q4-largest_decrease_table, echo = FALSE}
...
```{r q5-max_potential_increase, echo = FALSE}
...
```{r q5-max_potential_decrease, echo = FALSE}
...
```{r other-prepare_data_by_year_by_gender, cache = TRUE, echo = FALSE}
...
```{r other-plot_number_of_distinct_names, fig.width = 7, fig.height = 5, echo = FALSE}
...
```{r other-plot_total_counts, fig.width = 7, fig.height = 5, echo = FALSE}
...
by_year_coverage <- group_by(df, year, name) %>% summarize(tot_count = sum(count)) %>% arrange(year, desc(tot_count))
by_year_coverage <- group_by(df, year, name) %>% summarize(tot_count = sum(count)) %>% arrange(year, desc(tot_count)) %>% mutate()
```

```{r some_aggregation, cache = TRUE, echo = FALSE}
for_coverage2a <- group_by(df, year, name) %>% summarize(tot_count = sum(count)) %>% arrange(year, desc(tot_count)) %>% 
...
```{r other-plot_names_coverage_v2, fig.width = 7, fig.height = 5.5, echo = FALSE}
...
```{r other-plot_power_law_distribution_examples, fig.width = 7, fig.height = 10, echo = FALSE}
par(fig = c(0, 1, 0, 1), mar = c(5, 4, 4, 1)+0.1, cex.axis = 1.0)
single_panel_mar <- c(2, 2, 2, 1)
single_panel_oma <- c(0 ,0, 2, 0)
gr_par <- list( mar = single_panel_mar, oma = single_panel_oma, 
...
```{r other-plot-power_law_fits_slope_F, fig.width = 7, fig.height = 7, echo = FALSE}
...
```{r other-plot-power_law_fits_slope_M, fig.width = 7, fig.height = 7, echo = FALSE}
...
```{r other-plot-power_law_fits_slope_delta, fig.width = 7, fig.height = 7, echo = FALSE}
