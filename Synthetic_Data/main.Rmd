...
I report on the analysis and modeling of the provided dataset.
...
train <- read.delim(file = "data/codetest_train.txt.gz", stringsAsFactors = FALSE)
test  <- read.delim(file = "data/codetest_test.txt.gz", stringsAsFactors = FALSE)
```
...
```{r stats_NA_in_train-compute, echo = FALSE, cache = TRUE}
...
```{r stats_NA_in_test-compute, echo = FALSE, cache = TRUE}
...
colnames(df.NA_stats) <- c("n_NA", "n_Obs", "compl_col", "compl_row", 
                           "avrg_NA_by_col", "pct_NA_by_col", 
                           "avrg_NA_by_row", "pct_NA_by_row")

kable(df.NA_stats, digits = 2, caption = "Summary of NA Statistics by Data Set, Variables, Observations", align = "c")
```
...
```{r plot_NA_stats-1, echo = FALSE, fig.width = 6, fig.height = 6}
hstep <- 0.0025
# par(mfrow = c(2, 2))
# par(oma = c(0 ,0, 1, 0))
...
```{r plot_predictors_distributions_3x2-train_and_test, fig.width = 9, fig.height = 6, echo = FALSE}
...
```{r plot_predictors_summary_stats_train_vs_test_v2, fig.width = 7, fig.height = 6, echo = FALSE}
par(fig = c(0, 1, 0, 1))
single_panel_mar <- c(2, 2, 2, 1)
single_panel_oma <- c(0 ,0, 2, 0)
gr_par <- list( mar = single_panel_mar, oma = single_panel_oma, 
...
```{r plot_distributions_of_factors_train_vs_test, fig.width = 6, fig.height = 6, echo = FALSE}
...
```{r plot_target_distribution_v2, fig.width = 5, fig.height = 3.5, echo = FALSE}
gr_par <- list( mar = single_panel_mar, oma = single_panel_oma, 
                cex = 1.0, cex.axis = 0.8, cex.lab = 1.0, 
                las = 0, mgp = c(1.0, 0.0, 0),
                tcl = 0.3)
...
```{r deduplicated_corr_matrix_as_vector, echo = FALSE, cache = TRUE}
...
```{r plot_correlation_stats, fig.width = 8, fig.height = 3, echo = FALSE}
par(gr_par)
...
hist(corr1_pcomp_dedup$corr, freq = TRUE, breaks = seq(-1.0, 1.0, by = 0.02), 
     ...
     main = "Training - 'Zoomed' Distribution of\nCor(predictor, predictor)", cex.main = 0.9)
...
```{r correlated_variables-training, echo = FALSE}
...
```{r correlated_variables-testing, echo = FALSE}
...
```{r correlated_variables-with_target, echo = FALSE}
...
```{r scatterplot_target_vs_predictors_v2, fig.width = 8, fig.height = 6, echo = FALSE}
par(fig = c(0, 1, 0, 1))
single_panel_mar <- c(2, 2, 2, 1)
single_panel_oma <- c(0 ,0, 2, 0)
gr_par <- list( mar = single_panel_mar, oma = single_panel_oma, 
...
```{r outcome_vs_factors, echo = FALSE}
...
```{r figure-boxplot_outcome_vs_factors, fig.width = 8, fig.height = 6, echo = FALSE}
par(fig = c(0, 1, 0, 1))
...
```{r impute_train-correlation_matrix, cache = TRUE, echo = FALSE}
...
```{r impute_test-correlation_matrix, cache = TRUE, echo = FALSE}
...
```{r LM-diagnostic_plots, fig.width = 9, fig.height = 6.5, echo = FALSE}
par(fig = c(0, 1, 0, 1))
...
for( k in 1:length(seeds.train_model_MARS) ) {
    ...
    # mars.model <- earth(target ~ ., data = allTrain, degree = 2, nfold = 5, ncross = 6, keepxy = TRUE, pmethod = "cv")
...
```{r MARS-variable_importance-N4}
evimp(marsN4.model)
```

```{r MARS-predict_train-N4, eval = FALSE, echo = FALSE}
...
```{r MARS-get_used_predictors, echo = FALSE}
...
marsNall.imp_var
```
Note that some of them are factors: ` `r marsNall.imp_var[sapply(marsNall.imp_var, function(x) is.character(train[, x]))]` `.
...
```{r MARS-diagnostic_plots-N4, fig.width = 9, fig.height = 3.5, echo = FALSE}
par(fig = c(0, 1, 0, 1))
single_panel_mar <- c(3, 2, 2, 1)
single_panel_oma <- c(1 ,0, 1, 0)
gr_par <- list( mar = single_panel_mar, oma = single_panel_oma, 
...
```{r MARS-predict_test_set-ALL, cache = TRUE, echo = FALSE}
...
```{r MARS-merge_prediction_on_test_sets, cache = TRUE, echo = FALSE}
...
```{r MARS-plots-predicted_variables_minmax_and_MSE, fig.width = 10, fig.height = 3.5, echo = FALSE, cache = FALSE}
...
```{r write_prediction}
write.csv(summaryNall$target, file = "codetest_prediction.csv", row.names = FALSE, quote = FALSE)
```
...
```{r plot-predictors_summary_stats_train_vs_test_with_imp_vars, fig.width = 9, fig.height = 3.5, echo = FALSE}
par(fig = c(0, 1, 0, 1))
single_panel_mar <- c(2, 2, 2, 1)
single_panel_oma <- c(0 ,0, 2, 0)
gr_par <- list( mar = single_panel_mar, oma = single_panel_oma, 
...
```{r LM4-setup}
varnames_lm3_0005 <- c("(Intercept)", "f_35", "f_54", "f_61", "f_94", "f_175", "f_196", "f_205", "f_217", "f_218", "f_237")
...
```{r MARS-variable_importance-ALL}
for( i in 1:length(seeds.train_impute) ) {
    print( evimp(get(paste0("marsN", i, ".model"))) )
    cat("--------------------------------------------------------------------------------\n")
}
```
