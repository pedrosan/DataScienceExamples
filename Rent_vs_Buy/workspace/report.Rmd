...
```{r wilshire5000-show_prepared_data_list}
str(WW)
```
...
```{r sp500-prepare_data, cache = FALSE}
...
```{r sp500-show_prepared_data_list}
str(SP)
```
...
```{r nasdaq100-prepare_data, cache = FALSE}
...
```{r nasdaq100-show_prepared_data_list}
str(NN)
```
...
tmp <- dplyr::select(rr.df, date, time, WWidx_n, SPidx_n, NNidx_n) %>% gather(. , name, value, NNidx_n, SPidx_n, WWidx_n)
