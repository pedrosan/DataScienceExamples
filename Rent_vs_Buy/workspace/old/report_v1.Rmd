...
* _log return_: $${rl}_{i} = \log\left(\frac{x_i}{x_{i-1}}\right)$$
* _relative return_: $${rr}_i = e^{{rl}_i} - 1 = \frac{x_i - x_{i-1}}{x_{i-1}}$$
...
```{r wilshire5000-prepare_data-add_dates, cache = TRUE}
...
<button class="toggle_code">show hidden code</button>
```{r annual_returns_simulated_2, eval = TRUE, cache = TRUE, echo = TRUE}
# typical number of market days in a year is ~252
ndays <- 252
...
set.seed(1313)
test_rl.W5000 <- rep(0, N)
for(j in 1:N) {
    dummy <- rgl(ndays, WW$rl_fit_gl)
    test_rl.W5000[j] <- (cumprod(exp(dummy))[ndays] - 1.0)*100
}

test_rl.SP500 <- rep(0, N)
for(j in 1:N) {
    dummy <- rgl(ndays, SP$rl_fit_gl)
    test_rl.SP500[j] <- (cumprod(exp(dummy))[ndays] - 1.0)*100
}

test_rl.N100 <- rep(0, N)
for(j in 1:N) {
    dummy <- rgl(ndays, NN$rl_fit_gl)
    test_rl.N100[j] <- (cumprod(exp(dummy))[ndays] - 1.0)*100
}
```
...
<button class="toggle_code">show hidden code</button>
```{r plot-annual_returns_distr_2_rl, echo = TRUE, eval = TRUE, fig.width = 8.0, fig.height = 5.5}
xx2 <- seq(-200, 200, by = 1.0)
yy.W5000.n <- dnorm(xx2, mean = mean(test_rl.W5000), sd = sd(test_rl.W5000))
yy.SP500.n <- dnorm(xx2, mean = mean(test_rl.SP500), sd = sd(test_rl.SP500))
yy.N100.n  <- dnorm(xx2, mean = mean(test_rl.N100), sd = sd(test_rl.N100))
...
<button class="toggle_code">show hidden code</button>
```{r plot-annual_returns_timeseries_ggplot, echo = TRUE, fig.width = 10.0, fig.height = 5.5}
...
tmp <- dplyr::select(rr.df, date, time, WW, SP, NN) %>% gather(. , name, value, NN, SP, WW)
...
ggplot(data = tmp, aes(x = date, y = value)) + theme_bw() + 
       ...
       scale_colour_manual(values = c("red2", "blue2", "green4"), name = "Index", labels = c("NASDAQ100", "S&P500", "Wilshire5000"))
```
...
<button class="toggle_code">show hidden code</button>
```{r plot-indices_timeseries_ggplot_norm, echo = TRUE, eval = TRUE, fig.width = 10.0, fig.height = 5.5}
ggplot(data = rr.df, aes(x = time)) + theme_bw() + 
       theme(axis.title = element_text(size = 16),
             axis.text= element_text(size = 14),
             axis.line = element_line(size = 1)) +
